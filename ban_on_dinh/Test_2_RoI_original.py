...
def process_frame(img):
    try:
        ...
        ret, frame0 = cv2.threshold(frame, avg, 255, cv2.THRESH_BINARY)
        ...
        #     print(f"size :{size}")
        # for cnt in contours:
        #     # vẽ contour lên một mặt nạ
        #     mask = np.zeros_like(img)
        #     cv2.drawContours(mask, [cnt], 0, 255, -1)
            
        #     # trích xuất tất cả các tọa độ của các pixel trong contour
        #     coordinates = np.where(mask > 0)
        #     for x, y in zip(coordinates[1], coordinates[0]):
        #         # xử lý các tọa độ của các pixel trong contour ở đây
        #         print(x, y)
        ...
        #     pass
            ...
            array = a
            array_0 = b
            print(f"array[0]:{array[0]}, array[1]:{array[1]}, array[2]:{array[2]}, array[3]:{array[3]}")
