...
select = frame[frame > 60]
avg = np.mean(select)
sortSelect = np.sort(select)
print(f"sort:{sortSelect}")
print(f"avg:{avg}")
print(f"value of pixel > 60:{select}")
...
print("len sau khi loc",len(contours))
...
#        a00, a1, a2, a3, a4, a5, a6, a7, a8, a9 = xly.xu_ly_y(array2, values_y, row, threshold_code, input_var)
    ...
    # plt.scatter(values_x, mang_so_sanh)
    # plt.plot(values_x, mang_so_sanh)
    # plt.plot(values_x, values_y, color = 'blue')
    # plt.show()    

    print(f"mang_so_sanh = {mang_so_sanh[:]}")
    #s
    # array2 = [[0]]
    ...
    for j in range(0, c-d+1):  # c = 500, d= 10
        ...
        if n == 0:
            ...
            for o in range(0, input_var):
                # print(f"i:{i}, j:{j}, d:{d}, o:{o}, i+o:{i+o}, j+d-input_var+o:{j+d-input_var+o}")
