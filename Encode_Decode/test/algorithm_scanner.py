...
path = "venv/app_proccessing_image/ban_on_dinh/"
image = cv2.imread("venv/app_proccessing_image/ban_on_dinh/led_image/Led_debug1.jpg")
...
def transformation(image):
  ...
  for cnt in contours:
        ...
        if len(approx) == 4:  # Filter contours with 4 points
            simplified_contours.append(approx)
    #   simplified_contours.append(cv2.approxPolyDP(hull,
    #                             0.001*cv2.arcLength(hull,True),True))
        # print(f'check error simplified:{simplified_contours[0:10]}')
