...
def process_frame(img, heigth, width):
    ...
    # ret, frame0 = cv2.threshold(blurred, avg, 255, cv2.THRESH_BINARY)
...
def check_roi_tu_arr(str_x, Y, top, bot, Npixel):
    ...
    if abs(max2 - min2) >= 50*Npixel and gocLech <= 2  and str_[1] > 0 and all(x != 0 for x in str_outout2) :
        str_outout3[0] = (x_top2 + x_bot2)/2
        str_outout3[1] = min2
        str_outout3[2] = (x_top2 + x_bot2)/2
    ...
    elif abs(max2 - min2) >= 50*Npixel and gocLech > 2 and str_[1] > 0 and all(x != 0 for x in str_outout2):
...
img = cv2.imread("LED_Decode\led_image\\40cm_test2.jpeg")
height, width, channel = img.shape
print(f'height, width: {height}, {width}')
process_frame(img, height, width)
